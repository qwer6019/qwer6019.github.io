...
<p>
	...
	<h2>권위 증명(Proof of Authority)</h2>
	<h3>Reference: https://blockonomi.com/proof-of-authority/</h3><br>
	<b class="taxonomy">권위 증명(PoA)</b>은 지분 증명을 기반으로 하되 토큰을 스테이킹하는 대신 <b class="funccolor">아이덴티티</b>를 스테이킹의 형태로 활용하는 합의 알고리즘으로 이더리움의 공동 창립자인 Gavin Wood에 의해 제시되었다.
	<br>아이덴티티는 각 네트워크에서 거래와 블록을 검증하도록 승인받은 <b class="funccolor">검증자</b>들에 의해 스테이킹되며 검증자들은 네트워크의 효율성과 보안성을 보장하기 위해 일반적으로 적은 수(25명 이내)로 유지되므로 확장성이 뛰어나다. 이들은 공개 도메인의 교차 검증을 통해 사전 검증 가능한 방식으로 승인되므로(ex. 공증인 데이터베이스) 신뢰할 수 있다.
	<br><br>
	PoA의 주요 특징은 PoW 와 달리 연산 파워가 크게 요구되지 않고 노드들 사이에 합의를 위한 커뮤니케이션이 필요하지 않다는 것이다. 
	또한, PoS에서 주요 문제점이던 두 사용자의 지분이 동일하더라도 지분 가치가 전체 보유량에 따라 다를 수 있다는 것을 제거했다. 즉, 앨리스와 밥이 둘 다 1,000 토큰을 지분으로 가지더라도 앨리스가 1,000,000 토큰을, 밥이 10,000 토큰을 총 보유하고 있다면 밥의 지분이 전체 보유량 대비 크므로 앨리스보다 해당 네트워크의 성공에 더 많은 투자를 할 것이라는 것이다.
	<br><br>
	검증자가 되기 위한 요구사항들은 다음과 같으며 검증인들이 정직하게 행동하도록 유도하는 인센티브 구조를 가진다. 
	<ul>
		<li>검증자는 공개 도메인에서의 신뢰할 수 있는 데이터와 온체인에서의 아이덴티티가 <b class="funccolor">상호 참조</b>됨으로서 공식적으로 식별되어야 한다.</li>
		<li>장기간 검증자의 자격을 유지하는 것이 재정적 및 평판에 대한 분명한 인센티브임을 보장하기 위해 검증자의 <b class="funccolor">자격 요건</b>은 어려워야 한다.</li>
			<ul><li>네트워크에서 검증자로서 장기간 남아 있기 위한 재정적인 인센티브 제공 방식과 정직하게 행동하지 않는 경우의 패널티 부여 방식은 플랫폼들마다 다양하다.</li></ul>
		<li>검증자의 <b class="funccolor">선정 과정</b>은 완전히 일관되야 한다.</li>
	</ul> 
	악의적으로 행동하는 검증자를 검증 프로세스에서 쉽게 제거하고 대체 할 수 있으며, 해당 검증자는 공개적으로 평판이 추락하며 향후 재정적 손실을 입을 수 있다.
	현 인터넷 시대에서 공개적인 비난을 통해 평판을 완전히 잃는 것에 대한 위험성이 높아지는 것 또한 검증자가 시스템 내에서 정직하게 행동 하는 유인이 된다.
	<br><br>
	PoA 기반의 네트워크는 거래를 확인하고 합의하는 과정에서 개인 정보 보호와 효율성을 유지하면서도 검증자들이 해당 프로세스에서 상당한 영향력과 권한을 가지므로 권한이 부여 된 프라이빗 블록체인에 효과적이다. 
	<ul><li>각각이 자신의 검증자 역할을 하는 은행들의 네트워크</li></ul>
	하지만, 완전히 분권화되어 있지 않고 분산화와 중앙 집중화가 제공하는 효율성 간의 절충안이기 때문에 비트코인과 이더리움과 같은 진정한 분권화가 여전히 부족하다. 
	<ul>
		<li>검증자(ex. 은행)들이 네트워크를 사용하는 특정 공급 업체 또는 거래를 검열하거나 블랙 리스트를 작성할 수 있으므로 블록 체인의 정당성이 사라진다.</li>
	</ul>
</p>
<p class="quotes">
